...
join(Bucket, Nodes) ->
  ...
  {OkRep, _} =
    global:trans(
      {{?MODULE, Bucket}, self()},
      fun() ->
        gen_server:multi_call(CurrentNodes, Bucket, {join, AllNodes})
      end
    ),
...
leave(Bucket, Nodes) ->
  ...
  global:trans(
    {{?MODULE, Bucket}, self()},
    fun() ->
      gen_server:multi_call(CurrentNodes, Bucket, {leave, Nodes})
    end
  ),
  ok.
...
handle_call({join, Nodes}, _From, #state{nodes = CurrentNodes} = State) ->
  ...
  NewNodesOk =
    lists:foldl(
      fun(E, Acc) ->
        net_adm:ping(E) =:= pong andalso Acc
      end, true, NewNodes),
  case NewNodesOk of
    true ->
